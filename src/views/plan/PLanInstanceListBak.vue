<template>
  ...
  <el-drawer v-model="planInstanceVisible" direction="rtl" size="80%">
  ...
  <el-dialog v-model="tasksVisible" width="80%">
    <el-table :data="tasks" style="width: 100%" row-key="id" border>
      ...
      <el-table-column label="操作" width="200">
        <template #default="scope">
          <el-button link type="primary" @click="loadDetail(scope.$index, scope.row.planInstanceId)">查看内容</el-button>
          <el-button link type="primary" @click="() => {errorVisible = true;errorMsg.value = scope.row.errorMsg}">异常信息</el-button>
...
import {PlanStatusEnum, TriggerTypeEnum, JobStatusEnum, TaskTypeEnum} from '@/types/console-enums';
...
const planInstanceVisible = ref(false);
...
const loadDetail = (idx: number, planInstanceId: string) => {
  // 如果是普通的 抽屉
  planInstanceVisible.value = true
  loadJobInstances(planInstanceId)
  // 如果是 工作流 详情
}
